		...
		if ( ! class_exists( 'LearnPress' ) ) {
			...
			if ( is_array( $items ) && count( $items ) > 0 ) {
				for ( $index = 0; $index < count( $items ); $index ++ ) {
					switch ( $items[$index] ) {
						case 'courses':
							$menu_items_output .= '<li class="menu-item-courses"><a href="http://localhost/tirushop/my-account"><i class="lp-icon-my-courses"></i>' . esc_html__( 'Dashboard', 'eduma' ) . '</a></li>';
							break;
						case 'orders':
						...
						case 'become_a_teacher':
							if ( learn_press_get_page_link( 'become_a_teacher' ) && ! array_intersect( array( 'administrator', 'lp_teacher', 'instructor' ), $user->roles ) ) {
								$menu_items_output .= '<li class="menu-item-become-a-teacher"><a href="' . learn_press_get_page_link( 'become_a_teacher' ) . '"><i class="tk tk-user"></i>' . esc_html__( 'Become A Seller', 'eduma' ) . '</a></li>';
							}
							break;
						case 'certificates':
						...
						case 'wishlist':
							if ( class_exists( 'LP_Addon_Wishlist' ) ) {
								$menu_items_output .= '<li class="menu-item-wishlist"><a href="' . esc_url( $profile->get_tab_link( 'wishlist', true ) ) . '"><i class="tk tk-heart"></i>' . esc_html__( 'Wishlist', 'eduma' ) . '</a></li>';
							}
							break;
						case 'settings':
