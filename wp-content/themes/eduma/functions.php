...
const THIM_THEME_VERSION = '5.6.9';
...
add_action('init', 'thim_handle_popup_registration');
function thim_handle_popup_registration() {
	if (
		isset($_POST['eduma_register_user']) &&
		wp_verify_nonce($_POST['register_security'], 'ajax_register_nonce')
	) {
		...
		if ( empty($username) || empty($email) || empty($password) || empty($repeat) ) {
			wp_die(__('All fields are required.', 'eduma'));
		}

		if ( $password !== $repeat ) {
			wp_die(__('Passwords do not match.', 'eduma'));
		}

		if ( username_exists($username) || email_exists($email) ) {
			wp_die(__('Username or email already exists.', 'eduma'));
		}
